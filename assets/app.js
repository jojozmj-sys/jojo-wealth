  ...
  function bind() {
    ...
    function renderReviews() {
      ...
      if (advice && review.advice) {
        ...
        if (adviceBody) {
          var cards = [];
          // 兼容两种结构：数组 [{type,content},...]（gen_dailyreview.js 生成）或对象 {strategy/direction/risk}
          if (Array.isArray(a)) {
            var typeCls = { "仓位": "strategy", "策略": "strategy", "方向": "direction", "风险": "risk" };
            a.forEach(function(it) {
              if (!it || !it.content) return;
              var cls = typeCls[it.type] || "strategy";
              var title = it.type ? { "仓位": "📋 操作策略", "策略": "📋 操作策略", "方向": "🧭 关注方向", "风险": "⚠️ 风险提示" }[it.type] || it.type : "建议";
              cards.push('<div class="st-advice-card ' + cls + '">' +
                '<div class="ac-title">' + title + '</div>' +
                '<div class="ac-body">' + it.content + '</div></div>');
            });
          } else {
            cards = [
          ...
          adviceBody.innerHTML = cards.join("");
        }
      }
    }
